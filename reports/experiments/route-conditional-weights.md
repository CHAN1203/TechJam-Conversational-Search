...
Renumbered `E29 -> E31` when merged into `staging` on 2026-08-31: upstream had
already used E29 (slot-projected semantic query) and E30 (hard/soft constraint
separation) on `feat/hs` while this ran in parallel.

Per `docs/EXPERIMENT_WORKFLOW.md` step 6B, only this report and the ledger row
are merged. The implementation -- `route_semantic_weights`,
`route_popularity_weights` and `tests/test_route_weights.py` -- stays on
`experiment/route-conditional-weights` so a reviewer can cross-check it without
carrying rejected behaviour into the submission.
